...
public class Generator_Cuboid {
	...
	public static <GLO> void make(Polyhedron<GLO> poly, ValueMap map){
        ...
        if(!anyrem){
        	...
        	float yp = Generator.detached(rems, deuv, 2) && Generator.detached(rems, deuv, 3) ? 0 : d0;
        	float x0 = Generator.detached(rems, deuv, 1) ? 0 : d0;
        	float x1 = Generator.detached(rems, deuv, 2) ? 0 : w0;
        	float x2 = Generator.detached(rems, deuv, 4) ? 0 : w0;
        	float x3 = Generator.detached(rems, deuv, 0) ? 0 : d0;
